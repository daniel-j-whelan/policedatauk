...
def enable_police_api_logging(level=logging.INFO):
    logging.basicConfig(
        level=level,
        format="[%(asctime)s] [%(levelname)s] %(message)s",
    )
    logging.getLogger("police_api.retry").setLevel(level)
...
def async_retry(max_attempts=5, base_wait=1, max_wait=4):
    ...
    Args:
        ...
        base_wait (float): The base wait time (in seconds) for exponential backoff. Defaults to 0.5.
