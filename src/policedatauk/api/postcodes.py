...
class AsyncPostcodeAPI(BaseAsyncAPI):
    """Postcode-related asyncronous API methods for the Postcodes.io API."""
    ...
    async def get_postcode_info(
        ...
        try:
            response = await self.get(f"/{postcode}")
    ...
    async def is_valid_postcode(self, postcode: str) -> bool:
        ...
        """
        postcode = postcode.replace(" ", "").upper()
        response = await self.get(f"/{postcode}/validate")
        data = response.json()
        if data["status"] != 200:
            raise ValueError(f"API error: {data.get('error', 'Unknown')}")
...
class PostcodeAPI(BaseSyncAPI):
    """Postcode-related syncronous API methods for the Postcodes.io API."""
    ...
    def get_postcode_info(
        ...
        """
        postcode = postcode.replace(" ", "").upper()
        valid_postcode = self.is_valid_postcode(postcode)
        if not valid_postcode:
            # Need to create custom exception?
            raise ValueError(f"Invalid postcode: {postcode}")
        try:
            response = self.get(f"/{postcode}")
        except HTTPStatusError as e:
            raise ValueError(f"API error: {e}")
        data = response.json()
        if data["status"] != 200 or not data["result"]:
    ...
    def is_valid_postcode(self, postcode: str) -> bool:
        ...
        """
        postcode = postcode.replace(" ", "").upper()
        response = self.get(f"/{postcode}/validate")
        data = response.json()
        if data["status"] != 200:
            raise ValueError(f"API error: {data.get('error', 'Unknown')}")
        # Result is either True or False
